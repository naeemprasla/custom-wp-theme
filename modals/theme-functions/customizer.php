...
function vcsite_settings_js_scripts_customize_register($wp_customize)
{
    // Settings-------------------------
    $wp_customize->add_section(
        'vcsite_settings_jsscript_option',
        array(
            'title'       => __('CDN Options', 'vc-site-settings'),
            'priority'    => 120,
            'capability'  => 'edit_theme_options',
            'description' => __('', 'vc-site-settings'),
        )
    );

    

    $wp_customize->add_setting('enable_slick', array('default' => '1'));
    ...
    $wp_customize->add_control(
        new WP_Customize_Control(
            ...
            'enable_slick',
            array(
                'label'     => __('Enable Slick-Slider CDN', 'vc-site-settings'),
                'description' => __('Get Value by get_theme_mod(\'enable_slick\');', 'vc-site-settings'),
                'section'   => 'vcsite_settings_jsscript_option',
                'settings'  => 'enable_slick',
                'type'      => 'checkbox',
            )
        )
    );
    ...
    $wp_customize->add_control(
        new WP_Customize_Control(
            ...
            'enable_fontawesome',
            array(
                'label'     => __('Enable Font Awesome 5', 'vc-site-settings'),
...
add_action('customize_register', 'vcsite_settings_js_scripts_customize_register');
